...
/**
 * @brief       初始化IO输入引脚
 *   @note      IO_WAKE_UP_SIGNAL: PE0, 外部上拉电阻, 浮空输入模式
 *              IO_FAULT00_SIGNAL: PE1, 外部上拉电阻, 浮空输入模式
 *              IO_DI5_SIGNAL: PE2, 外部上拉电阻, 浮空输入模式
 * @param       无
 * @retval      无
 */
void io_in_init(void) {
    ...
    /* IO_DI5_SIGNAL -> PE2 */
    gpio_init_struct.Pin = IO_DI5_SIGNAL_GPIO_PIN;
    HAL_GPIO_Init(IO_DI5_SIGNAL_GPIO_PORT, &gpio_init_struct);
}
...
/**
 * @brief       读取DI5信号
 *   @note      直接读取GPIO寄存器，返回当前DI5信号电平状态
 * @param       无
 * @retval      1: DI5信号有效, 0: DI5信号无效
 */
uint8_t io_read_di5(void) { return (HAL_GPIO_ReadPin(IO_DI5_SIGNAL_GPIO_PORT, IO_DI5_SIGNAL_GPIO_PIN) == GPIO_PIN_SET) ? 1 : 0; }
